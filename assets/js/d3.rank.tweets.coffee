...
class TweetsRank extends @app.LabelRank
  ...
  buildLabel: (label)->
    ...
    label.append('foreignObject').attr("class", "tweet-text-box")
                          ...
                          .html (d) =>
                            sourceForD = @source[d.name]
                            sourceText = if sourceForD
                              sourceForD.text
                            else
                              "Unable to find source for #{d.name}"
                            '<div class="tweet-text"><textarea disabled readonly style="width: 287px; height: 85px; padding-top: 0px;">' + sourceText + '</textarea></div>'
  ...
  redrawLabel: ->
    ...
    @svg.selectAll("image").data(@data)
      ...
      .attr("x", 20).attr("y", 0)
      .attr "xlink:href", (d) =>
        sourceForName = @source[d.name]
        if sourceForName
          @source[d.name].user.profile_image_url
        else
          "Couldnt find tweet #{d.name}"
